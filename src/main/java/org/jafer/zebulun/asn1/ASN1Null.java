 ...
 *
 * This class represents a null value. A NULL is used when only the tag is of
 * interest, and not any value.
 *
...
public final class ASN1Null extends ASN1Any {
  ...
  //----------------------------------------------------------------
  /**
   * Default constructor for an ASN.1 NULL object. The tag is set to the default
   * tag of UNIVERSAL 5. A NULL has no value.
   */
  public ASN1Null() {
  }
  ...
  public void
    ...
    if (check_tag) {
      if (ber_enc.tag_get() != TAG
        ...
        throw new ASN1EncodingException("ASN.1 NULL: bad BER: tag=" + ber_enc.tag_get()
                + " expected " + TAG + "\n");
      }
    }
  ...
  public static class XER_Parser_Proxy extends XERsaxHandler.XER_Parser_Proxy {
    ...
    private static final int STATE_TERM = 2;
