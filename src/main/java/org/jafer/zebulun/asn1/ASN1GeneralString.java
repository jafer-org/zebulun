 ...
 * ASN.1 GeneralString
 *
 * The <code>GeneralString</code> type denotes an arbitrary string of General
...
public class ASN1GeneralString extends ASN1OctetString {
  ...
  //----------------------------------------------------------------
  /**
   * Constructor for a GeneralString object. It sets the tag to the default
   * value of UNIVERSAL 27 (0x1B).
   *
  ...
  public ASN1GeneralString(BEREncoding ber, boolean check_tag)
    ...
    if (check_tag) {
      if (ber.tag_get() != TAG
        ...
        throw new ASN1EncodingException("ASN.1 GeneralString: bad BER: tag=" + ber.tag_get()
                + " expected " + TAG + "\n");
      }
    }
  }
